...
PrismUpdaterApp::PrismUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Penguin Launcher"));
    ...
    if (!dirParam.isEmpty()) {
        // the dir param. it makes penguin launcher data path point to whatever the user specified
        // on command line
...
void PrismUpdaterApp::run()
{
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Penguin Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
    ...
    if (update_lock.exists()) {
        ...
        auto infoMsg =
            tr("This installation has a update lock file present at: %1\n"
               ...
               "proceeding.\n"
               "Check the Penguin Launcher updater log at: \n"
               "%7\n"
...
void PrismUpdaterApp::backupAppDir()
{
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "PenguinLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "penguinlauncher.exe",
                "penguinlauncher_filelink.exe",
                "penguinlauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
