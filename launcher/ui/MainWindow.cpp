    ...
    for (auto& url : urls) {
        ...
        if (!url.isLocalFile()) {  // download the remote resource and identify
            ...
            if (url.scheme() == "curseforge" || (url.scheme() == BuildConfig.LAUNCHER_APP_BINARY_NAME && url.host() == "install")) {
                ...
                continue;
            } else if ((url.scheme() == "penguinlauncher" || url.scheme() == BuildConfig.LAUNCHER_APP_BINARY_NAME) && isExternalURLImport) {
                // PenguinLauncher URL protocol modpack import
                // works for any prism fork
                // preferred import format: penguinlauncher://import?url=ENCODED
                ...
                // alternative import format: penguinlauncher://import/ENCODED
...
void MainWindow::globalSettingsClosed()
{
    ...
    // but Penguin Launcher exits abnormally, causing the window state to never be saved:
